...
from epigraph_ph.phase2.latent_temporal_graph import _build_sample_arrays, _phase15_phi_by_block, _weighted_standardize
...
def _hidden_mode_artifact(
    *,
    ...
    if standardized_design.size == 0:
        return {"available": False, "reason": "empty_design_matrix"}, None
    u, singular_values, _ = np.linalg.svd(low_rank_adjacency, full_matrices=False)
    rank = min(estimated_rank, int(u.shape[1]))
    if rank <= 0:
        return {"available": False, "reason": "zero_hidden_rank"}, None
    hidden_scores = standardized_design @ (u[:, :rank] * singular_values[:rank])
...
def build_phase2_structural_artifacts(
    *,
    ...
    direct_rows = _edge_summary_rows(
        dict(latent_bundle.get("scales") or {}),
    ...
    multiscale_rows = [
        ...
        for row in list(multiscale_blankets.get("factor_support_rows") or [])
    ]

...
def _save_overview(rows: list[dict[str, Any]], path: Path) -> None:
    if not rows:
        suite._plot_placeholder(path, title="Exact calibration batch", body="No rows.")
        return
    ...
    ax.set_title("TR-V3 exact calibration batch")
...
def _markdown_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "| Experiment | Quarterly MAE | Baseline MAE | Raw diagnosed MAE | Raw ART MAE | Raw flow MAE | Suppression honesty |",
        "|---|---:|---:|---:|---:|---:|---|",
    ]
    for row in list(payload.get("rows") or []):
        lines.append(
            f"| {row['experiment_id']} | {float(row['quarterly_mean_mae']):.6f} | {float(row['quarterly_baseline_mae']):.6f} | "
            f"{float(row['diagnosed_raw_mae']):.3f} | {float(row['art_raw_mae']):.3f} | {float(row['flow_raw_mae']):.3f} | "
    ...
    lines.extend(
        [
            ...
            "## Lockbox",
            "",
            f"- Winner: `{payload['lockbox']['winner']['experiment_id']}`",
            f"- MAE: `{float(payload['lockbox']['winner']['quarterly_mean_mae']):.6f}`",
        ]
    )
    return "\n".join(lines) + "\n"


def run_tr_v3_exact_calibration_batch(
    *,
    run_id: str,
    ...
    rows = [
        _result_row(result_map["EXP-R10-M1-F1"]),
        _result_row(result_map["EXP-R10-M1-B1"]),
        _result_row(result_map["EXP-R10-M1-F1-C1"]),
    ]
    ...
    observation_rows = publish._observation_rows_for_lockbox(archive_run, contract_name="exact_only", holdout_years=holdout_years)
    scoring_tiers = publish._scoring_tiers("exact_only")
    spec_map = {spec.experiment_id: spec for spec in suite.build_experiment_suite_specs()}
    lockbox_rows = []
    for experiment_id in ("EXP-R10-M1-F1", "EXP-R10-M1-B1", "EXP-R10-M1-F1-C1", "EXP-R1"):
        lockbox_rows.append(
    ...
    lockbox_winner = min(lockbox_rows, key=lambda row: float(row["quarterly_mean_mae"]))

    incumbent = next(row for row in rows if row["experiment_id"] == "EXP-R10-M1-F1")
    challenger = next(row for row in rows if row["experiment_id"] == "EXP-R10-M1-F1-C1")
    if float(challenger["quarterly_mean_mae"]) < float(incumbent["quarterly_mean_mae"]):
        decision = {
            "winner_id": "EXP-R10-M1-F1-C1",
            "status": "promote",
            "why": "Crossfit diagnosed calibration improved the primary exact-only MAE.",
        }
    elif (
        ...
        decision = {
            ...
            "why": "Crossfit diagnosed calibration did not beat the incumbent exact winner on the primary exact-only contract.",
        }

    overview_path = analysis_dir / "exact_calibration_overview.png"
    ...
    report_payload = {
        ...
        "lockbox": {
            "rows": lockbox_rows,
            "winner": {
                "experiment_id": str(lockbox_winner["experiment_id"]),
                "quarterly_mean_mae": float(lockbox_winner["quarterly_mean_mae"]),
            },
        },
        "decision": decision,
        "artifacts": {"overview_graph": overview_path.name},
    }
    write_json(analysis_dir / "tr_v3_exact_calibration_batch_report.json", report_payload)
    (analysis_dir / "tr_v3_exact_calibration_batch_report.md").write_text(_markdown_report(report_payload), encoding="utf-8")
    return report_payload


def main() -> None:
    parser = argparse.ArgumentParser(description="Run targeted TR-V3 exact calibration comparisons.")
    ...
    run_tr_v3_exact_calibration_batch(

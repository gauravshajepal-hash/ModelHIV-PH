...
from epigraph_ph.phase3.shared.evaluation_regimes import BROAD_FROZEN_HISTORY_REGIME
...
def _archive_point_year(point: dict[str, Any]) -> int:
    ...
    if year > 0:
        return year
    for key in ("effective_month", "month", "source_month"):
        year = _month_year(str(point.get(key) or "")) or 0
        if year > 0:
            return int(year)
    return 0


def _archive_points_from_program_points(
    program_points: list[dict[str, Any]],
    years: list[int],
    month_axis: list[str],
) -> list[dict[str, Any]]:
    ...
    for raw_point in program_points:
        point = dict(raw_point)
        year = _archive_point_year(point)
        if year not in year_set:
            continue
        if any(point.get(key) in {None, ""} for key in ("estimated_plhiv", "diagnosed", "on_art", "viral_load_tested", "suppressed")):
            continue
        fallback_month = _latest_month_for_year(month_axis, year) if year > 0 else ""
        month = str(point.get("month") or point.get("effective_month") or point.get("source_month") or "")
        effective_month = str(point.get("effective_month") or month or point.get("source_month") or "")
        source_month = str(point.get("source_month") or month or effective_month or "")
        if month and month not in month_set:
            month = ""
        if effective_month and effective_month not in month_set:
            effective_month = ""
        if source_month and source_month not in month_set:
            source_month = ""
        resolved_month = month or effective_month or source_month or fallback_month
        resolved_effective_month = effective_month or resolved_month or fallback_month
        resolved_source_month = source_month or resolved_month or resolved_effective_month or fallback_month
        if not resolved_month or not resolved_effective_month:
            continue
        points.append(
            {
                **point,
                ...
                "effective_month": resolved_effective_month,
                "source_month": resolved_source_month,
                "temporal_precision": str(point.get("temporal_precision") or "annual_snapshot"),
                "estimated_plhiv": float(point.get("estimated_plhiv") or 0.0),
                "diagnosed": float(point.get("diagnosed") or 0.0),
                "on_art": float(point.get("on_art") or 0.0),
                "viral_load_tested": float(point.get("viral_load_tested") or 0.0),
                "suppressed": float(point.get("suppressed") or 0.0),
            }
        )
    points.sort(
        key=lambda item: (
            int(item.get("year") or 0),
            _month_ordinal(str(item.get("effective_month") or item.get("month") or "")) or -1,
...
def _load_archive_program_points(archive_dir: Path) -> list[dict[str, Any]]:
    payload = read_json(archive_dir / "harp_program_points.json", default={})
    if isinstance(payload, dict):
        return [dict(row) for row in list(payload.get("points") or [])]
    if isinstance(payload, list):
        return [dict(row) for row in payload]
    return []


def _load_archive_diagnosis_flow_points(archive_dir: Path) -> list[dict[str, Any]]:
...
def _diagnosis_flow_point_year(point: dict[str, Any]) -> int:
    for key in ("period_end", "effective_month", "month", "source_month"):
        year = _month_year(str(point.get(key) or "")) or 0
        if year > 0:
            return int(year)
    raw_year = point.get("year")
    try:
        ...
        resolved_year = 0
    return resolved_year if resolved_year > 0 else 0


def _filter_diagnosis_flow_points_by_years(
    points: list[dict[str, Any]],
    years: list[int],
) -> list[dict[str, Any]]:
    year_set = {int(year) for year in years}
    filtered = [dict(point) for point in points if _diagnosis_flow_point_year(dict(point)) in year_set]
    filtered.sort(
        key=lambda item: (
            _month_ordinal(str(item.get("period_end") or item.get("effective_month") or item.get("month") or "")) or -1,
            str(item.get("metric_name") or item.get("label") or ""),
        )
    )
    return filtered
...
def _run_phase3_build_legacy(
    *,
    ...
    markov_blanket = dict(phase2_payload.get("markov_blanket") or {})
    eligibility_surfaces = dict(phase2_payload.get("eligibility_surfaces") or {})
    core_tensor = np.asarray(phase2_payload.get("core_feature_tensor_array"), dtype=np.float32)
...
def _prepare_frozen_backtest_inputs(
    *,
    ...
    archive_program_points = _load_archive_program_points(archive_dir)
    archive_diagnosis_flow_points = _load_archive_diagnosis_flow_points(archive_dir)
    ...
    dense_full_month_axis = _contiguous_month_axis(full_month_axis)
    dense_standardized_tensor = _densify_standardized_tensor(standardized_tensor, full_month_axis, dense_full_month_axis)
    month_indices = _month_indices_through_year(dense_full_month_axis, max(resolved_train_years))
    if not month_indices:
        raise RuntimeError("no phase1 months were retained after applying the frozen-history training cutoff")
    filtered_month_axis = [dense_full_month_axis[idx] for idx in month_indices]
    ...
    filtered_tensor = np.asarray(dense_standardized_tensor[:, month_indices, :], dtype=np.float32)

    train_harp_points = _archive_points_from_program_points(archive_program_points, resolved_train_years, dense_full_month_axis)
    holdout_harp_points = _archive_points_from_program_points(archive_program_points, resolved_holdout_years, dense_full_month_axis)
    train_diagnosis_flow_points = _filter_diagnosis_flow_points_by_years(archive_diagnosis_flow_points, resolved_train_years)
    holdout_diagnosis_flow_points = _filter_diagnosis_flow_points_by_years(archive_diagnosis_flow_points, resolved_holdout_years)
    if not train_harp_points or not holdout_harp_points:
        raise RuntimeError("historical HARP archive does not contain complete train/holdout program points")
    official_points = [
        dict(point)
    ...
    holdout_eval_months = [str(point.get("month") or _latest_month_for_year(dense_full_month_axis, int(point.get("year") or 0))) for point in holdout_harp_points]
    ...
    backtest_config = {
        ...
        "train_diagnosis_flow_points": train_diagnosis_flow_points,
        "holdout_diagnosis_flow_points": holdout_diagnosis_flow_points,
    }
    return {
        "ctx": ctx,
    ...
    *,
    ...
    manifest = run_phase3_rescue_core(
        ...
        reference_overrides={
            ...
            "diagnosis_flow_points": prepared["backtest_config"]["train_diagnosis_flow_points"],
        },
...
def run_phase3_frozen_backtest_tournament(
    *,
    ...
    for representation in representations:
        ...
        trial_rows.append(
            {
                "evaluation_regime": BROAD_FROZEN_HISTORY_REGIME,
                ...
                "simple_compartmental_mean_absolute_error": float(evaluation_summary.get("simple_compartmental_mean_absolute_error", 0.0)),
...
def run_phase3_frozen_backtest_tuning(
    *,
    ...
    full_month_axis = list(axis_catalogs.get("month", []))
    if not full_month_axis:
        raise RuntimeError("phase1 month axis is missing; cannot run frozen-history tuning")
    dense_full_month_axis = _contiguous_month_axis(full_month_axis)
    dense_standardized_tensor = _densify_standardized_tensor(standardized_tensor, full_month_axis, dense_full_month_axis)
    month_indices = _month_indices_through_year(dense_full_month_axis, max(train_years))
    if not month_indices:
        raise RuntimeError("no phase1 months were retained after applying the frozen-history tuning cutoff")
    filtered_month_axis = [dense_full_month_axis[idx] for idx in month_indices]
    filtered_axis_catalogs = dict(axis_catalogs)
    filtered_axis_catalogs["month"] = filtered_month_axis
    filtered_rows = _filter_rows_through_year(normalized_rows, max(train_years))
    filtered_tensor = np.asarray(dense_standardized_tensor[:, month_indices, :], dtype=np.float32)
    train_harp_points = _archive_points_from_program_points(archive_program_points, train_years, dense_full_month_axis)
    holdout_harp_points = _archive_points_from_program_points(archive_program_points, holdout_years, dense_full_month_axis)
    train_diagnosis_flow_points = _filter_diagnosis_flow_points_by_years(archive_diagnosis_flow_points, train_years)
    holdout_diagnosis_flow_points = _filter_diagnosis_flow_points_by_years(archive_diagnosis_flow_points, holdout_years)
    official_points = [
        dict(point)
        for point in OFFICIAL_REFERENCE_POINTS
    ...
    def _run_candidates(candidates: list[dict[str, Any]]) -> bool:
        ...
        for candidate in candidates:
            ...
            manifest = run_phase3_rescue_core(
                ...
                reference_overrides={
                    "official_points": official_points,
                    "harp_points": train_harp_points,
                    "diagnosis_flow_points": train_diagnosis_flow_points,
                },
                backtest_config=backtest_config,

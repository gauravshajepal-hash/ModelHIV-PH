...
from epigraph_ph.phase3.shared.evaluation_regimes import (
    BROAD_FROZEN_HISTORY_REGIME,
    broad_frozen_tournament_artifact_path,
    load_broad_representation_trial_rows,
)
...
from epigraph_ph.phase3.shared.temporal_scaffold import (
    temporal_representation_bundle,
)
...
def _phase3_frozen_backtest_section(key: str) -> dict[str, Any]:
    frozen_cfg = dict(_phase3_prior("frozen_backtest") or {})
    section = dict(frozen_cfg.get(key, {}) or {})
    if not section:
        raise KeyError(f"missing hiv phase3 frozen_backtest['{key}']")
    return section


def _phase3_death_scale() -> float:
    return float(dict(_phase3_constraint("torch_initialization") or {})["death_scale"])


def _phase3_death_ceiling() -> float:
    return float(dict(_phase3_constraint("torch_initialization") or {})["death_ceiling"])


def _predict_deaths_np(loss_share: np.ndarray) -> np.ndarray:
    return np.clip(np.asarray(loss_share, dtype=np.float32) * _phase3_death_scale(), 0.0, _phase3_death_ceiling()).astype(np.float32)


def _predict_deaths_torch(loss_share: Any) -> Any:
    return torch.clamp(loss_share * float(_phase3_death_scale()), 0.0, float(_phase3_death_ceiling()))


def _predict_deaths_jax(loss_share: Any) -> Any:
    return jnp.clip(loss_share * float(_phase3_death_scale()), 0.0, float(_phase3_death_ceiling()))
...
def _temporary_reference_points(
    *,
    ...
    diagnosis_flow_points: list[dict[str, Any]] | None = None,
):
    global OFFICIAL_REFERENCE_POINTS, HARP_PROGRAM_POINTS, DIAGNOSIS_FLOW_POINTS
    previous_official = OFFICIAL_REFERENCE_POINTS
    previous_harp = HARP_PROGRAM_POINTS
    previous_diagnosis_flow = DIAGNOSIS_FLOW_POINTS
    ...
    if diagnosis_flow_points is not None:
        DIAGNOSIS_FLOW_POINTS = [dict(item) for item in diagnosis_flow_points]
    try:
        yield
    finally:
        OFFICIAL_REFERENCE_POINTS = previous_official
        HARP_PROGRAM_POINTS = previous_harp
        DIAGNOSIS_FLOW_POINTS = previous_diagnosis_flow


def _archive_harp_program_points(run_dir: Path) -> list[dict[str, Any]]:
    payload = read_json(run_dir / "harp_archive" / "harp_program_points.json", default={})
    points = list(dict(payload).get("points", []) or [])
    return [dict(point) for point in points if isinstance(point, dict)]


def _archive_diagnosis_flow_points(run_dir: Path) -> list[dict[str, Any]]:
    payload = read_json(run_dir / "harp_archive" / "diagnosis_flow_points.json", default={})
    points = list(dict(payload).get("points", []) or [])
    return [dict(point) for point in points if isinstance(point, dict)]


def _nearest_month_index(month_axis: list[str], month_label: str) -> int | None:
    target_ordinal = _month_ordinal(month_label)
    if target_ordinal is None or not month_axis:
        return None
    month_ordinals = [_month_ordinal(label) for label in month_axis]
    valid = [idx for idx, ordinal in enumerate(month_ordinals) if ordinal is not None]
    if not valid:
        return None
    exact_lookup = {label: idx for idx, label in enumerate(month_axis)}
    exact = exact_lookup.get(str(month_label))
    if exact is not None:
        return int(exact)
    return min(valid, key=lambda idx: abs(int(month_ordinals[idx] or 0) - int(target_ordinal)))


def _diagnosis_flow_reference_arrays(month_axis: list[str]) -> dict[str, Any]:
    probability_eps = float(_phase3_stabilizer("probability_eps"))
    start_indices: list[int] = []
    end_indices: list[int] = []
    target_share: list[float] = []
    weights: list[float] = []
    rows: list[dict[str, Any]] = []
    for point in DIAGNOSIS_FLOW_POINTS:
        start_label = str(point.get("period_start") or point.get("month") or point.get("effective_month") or "")
        end_label = str(point.get("period_end") or point.get("effective_month") or point.get("month") or "")
        start_idx = _nearest_month_index(month_axis, start_label)
        end_idx = _nearest_month_index(month_axis, end_label)
        target_value = float(point.get("diagnosed_share") or 0.0)
        if start_idx is None or end_idx is None or target_value <= 0.0:
            continue
        resolved_start = int(min(start_idx, end_idx))
        resolved_end = int(max(start_idx, end_idx))
        weight_value = max(float(point.get("evidence_confidence") or 0.0), probability_eps)
        start_indices.append(resolved_start)
        end_indices.append(resolved_end)
        target_share.append(target_value)
        weights.append(weight_value)
        rows.append(
            {
                "label": str(point.get("label") or f"Diagnosis flow {end_label}"),
                "period_start": start_label,
                "period_end": end_label,
                "resolved_period_start": month_axis[resolved_start],
                "resolved_period_end": month_axis[resolved_end],
                "target_diagnosed_share": round(target_value, 6),
                "weight": round(weight_value, 6),
                "months_covered": int(point.get("months_covered") or (resolved_end - resolved_start + 1)),
                "advanced_hiv_share": round(float(point.get("advanced_hiv_share") or 0.0), 6) if point.get("advanced_hiv_share") is not None else None,
                "source_id": str(point.get("source_id") or ""),
                "source_label": str(point.get("source_label") or ""),
                "source_url": str(point.get("source_url") or ""),
            }
        )
    return {
        "start_index": np.asarray(start_indices, dtype=np.int32),
        "end_index": np.asarray(end_indices, dtype=np.int32),
        "target_share": np.asarray(target_share, dtype=np.float32),
        "weight": np.asarray(weights, dtype=np.float32),
        "rows": rows,
    }


def _diagnosis_flow_segment_sums_jax(series: Any, start_index: Any, end_index: Any) -> Any:
    prefix = jnp.concatenate([jnp.zeros((1,), dtype=series.dtype), jnp.cumsum(series, axis=0)], axis=0)
...
def _resolved_point_month(point: dict[str, Any], month_axis: list[str]) -> tuple[str | None, str]:
    return point_effective_month(point, month_axis)


def _contiguous_month_axis(month_axis: list[str]) -> list[str]:
    ordinals = sorted({int(ordinal) for ordinal in (_month_ordinal(label) for label in month_axis) if ordinal is not None})
    if not ordinals:
        return list(month_axis)
    return [_month_label_from_ordinal(ordinal) for ordinal in range(ordinals[0], ordinals[-1] + 1)]


def _densify_standardized_tensor(standardized_tensor: np.ndarray, month_axis: list[str], dense_month_axis: list[str]) -> np.ndarray:
    if standardized_tensor.ndim != 3 or standardized_tensor.shape[1] != len(month_axis) or dense_month_axis == month_axis:
        return np.asarray(standardized_tensor, dtype=np.float32)
    dense = np.zeros((standardized_tensor.shape[0], len(dense_month_axis), standardized_tensor.shape[2]), dtype=np.float32)
    dense_lookup = {label: idx for idx, label in enumerate(dense_month_axis)}
    for source_idx, label in enumerate(month_axis):
        target_idx = dense_lookup.get(label)
        if target_idx is None:
            continue
        dense[:, target_idx, :] = np.asarray(standardized_tensor[:, source_idx, :], dtype=np.float32)
    return dense.astype(np.float32)


def _regrid_time_tensor_to_axis(tensor: np.ndarray, source_month_axis: list[str], target_month_axis: list[str]) -> np.ndarray:
    tensor_np = np.asarray(tensor, dtype=np.float32)
    if (
        tensor_np.ndim != 3
        or tensor_np.shape[1] != len(source_month_axis)
        or not source_month_axis
        or not target_month_axis
        or source_month_axis == target_month_axis
    ):
        return tensor_np
    source_ordinals = np.asarray(
        [_month_ordinal(label) if _month_ordinal(label) is not None else -1 for label in source_month_axis],
        dtype=np.float32,
    )
    target_ordinals = np.asarray(
        [_month_ordinal(label) if _month_ordinal(label) is not None else -1 for label in target_month_axis],
        dtype=np.float32,
    )
    valid_source = source_ordinals >= 0
    valid_target = target_ordinals >= 0
    if not np.any(valid_source) or not np.any(valid_target):
        return tensor_np
    aligned = np.zeros((tensor_np.shape[0], len(target_month_axis), tensor_np.shape[2]), dtype=np.float32)
    xp = source_ordinals[valid_source]
    for province_idx in range(tensor_np.shape[0]):
        for feature_idx in range(tensor_np.shape[2]):
            fp = tensor_np[province_idx, valid_source, feature_idx].astype(np.float32)
            if fp.size == 0:
                continue
            aligned_values = np.interp(target_ordinals[valid_target], xp, fp, left=float(fp[0]), right=float(fp[-1]))
            aligned[province_idx, valid_target, feature_idx] = aligned_values.astype(np.float32)
...
def _official_anchor_curves(month_axis: list[str]) -> dict[str, np.ndarray]:
    ...
    for point in OFFICIAL_REFERENCE_POINTS:
        resolved_month, _ = _resolved_point_month(point, month_axis)
        ordinal = _month_ordinal(resolved_month or "")
        if ordinal is None:
            continue
...
def build_archive_aware_observation_targets(
    *,
    standardized_tensor: np.ndarray,
    normalized_rows: list[dict[str, Any]],
    parameter_catalog: list[dict[str, Any]],
    ...
    resolved_harp_points = [dict(item) for item in list(harp_program_points or [])]
    resolved_diagnosis_flow_points = [dict(item) for item in list(diagnosis_flow_points or [])]
    if run_dir is not None:
        if not resolved_harp_points:
            resolved_harp_points = _archive_harp_program_points(run_dir)
        if not resolved_diagnosis_flow_points:
            resolved_diagnosis_flow_points = _archive_diagnosis_flow_points(run_dir)
    with _temporary_reference_points(
        harp_program_points=resolved_harp_points if resolved_harp_points else None,
        diagnosis_flow_points=resolved_diagnosis_flow_points if resolved_diagnosis_flow_points else None,
    ):
        observation_ladder, observation_targets, _target_rows = build_observation_ladder(
        ...
        observation_ladder, observation_targets, target_rows, harp_summary = _inject_harp_program_targets(
            observation_ladder=observation_ladder,
            observation_targets=observation_targets,
            province_axis=province_axis,
            month_axis=month_axis,
        )
        diagnosis_flow_arrays = _diagnosis_flow_reference_arrays(month_axis)
    support_summary = {
        "month_axis_start": month_axis[0] if month_axis else "",
        "month_axis_end": month_axis[-1] if month_axis else "",
        "month_axis_count": int(len(month_axis)),
        "harp_program_point_count": int(len(resolved_harp_points)),
        "diagnosis_flow_point_count": int(len(resolved_diagnosis_flow_points)),
        "harp_program_support": dict(harp_summary),
        "diagnosis_flow_summary": {
            "point_count": int(len(list(diagnosis_flow_arrays.get("rows", []) or []))),
            "rows": [dict(row) for row in list(diagnosis_flow_arrays.get("rows", []) or [])],
        },
    }
    return observation_ladder, observation_targets, target_rows, support_summary
...
def _default_factor_transition_hooks(row: dict[str, Any]) -> list[str]:
    hooks: list[str] = []
    network_family = str(row.get("network_feature_family") or "").strip().lower()
    block_name = str(row.get("block_name") or row.get("interpretability_label") or "").strip().lower()
    if network_family == "information_propagation":
        hooks.extend(["diagnosis_transitions", "linkage_transitions"])
    elif network_family == "reaction_diffusion":
        hooks.extend(["diagnosis_transitions", "linkage_transitions", "retention_attrition_transitions"])
    elif network_family == "percolation_fragility":
        hooks.extend(["diagnosis_transitions", "retention_attrition_transitions", "suppression_transitions"])
    if block_name in {"stigma_behavior_information", "behavior"}:
        hooks.extend(["diagnosis_transitions", "linkage_transitions"])
    elif block_name in {"service_delivery_infrastructure", "policy_implementation", "policy", "logistics_access", "logistics"}:
        hooks.extend(["diagnosis_transitions", "linkage_transitions", "retention_attrition_transitions"])
    elif block_name in {"economics_affordability", "economics"}:
        hooks.extend(["linkage_transitions", "retention_attrition_transitions"])
    elif block_name in {"epidemiology_cascade", "population_structure_demography", "population"}:
        hooks.extend(["diagnosis_transitions"])
    ordered: list[str] = []
    seen: set[str] = set()
    for hook in hooks:
        if hook not in seen:
...
def _merge_multiscale_blanket_factor_rows(
    run_dir: Any,
    factor_catalog: list[dict[str, Any]],
    promoted: list[dict[str, Any]],
    supporting: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    blanket_summary = read_json(run_dir / "phase2" / "multiscale_phase3_target_blankets.json", default={})
    factor_ids = [
        str(value)
        for value in list(blanket_summary.get("merged_target_factor_ids") or blanket_summary.get("merged_blanket_factor_ids") or [])
        if str(value)
    ]
    if not factor_ids:
        return promoted, supporting
    factor_lookup = {str(row.get("factor_id") or ""): dict(row) for row in factor_catalog}
    existing = {str(row.get("factor_id") or "") for row in [*promoted, *supporting]}
    merged_supporting = list(supporting)
    for factor_id in factor_ids:
        if factor_id in existing or factor_id not in factor_lookup:
            continue
        row = dict(factor_lookup[factor_id])
        row["promotion_class"] = "multiscale_supported"
        row["selection_variant"] = "phase2_multiscale"
        merged_supporting.append(row)
        existing.add(factor_id)
    return list(promoted), merged_supporting
...
def _build_mesoscopic_modifier_covariates(
    *,
    ...
    factor_axes = read_json(run_dir / "phase15" / "multiscale_factor_axes.json", default={})
    factor_month_axis = list(factor_axes.get("month") or [])
    if factor_tensor.shape[0] != base.shape[0] or factor_tensor.shape[1] != base.shape[1]:
        if factor_tensor.shape[0] == base.shape[0] and factor_month_axis and len(factor_month_axis) == factor_tensor.shape[1]:
            factor_tensor = _regrid_time_tensor_to_axis(factor_tensor, factor_month_axis, month_axis)
            alignment_notes.append(
                f"factor_tensor_regridded_to_phase3_month_axis:{len(factor_month_axis)}->{len(month_axis)}"
            )
        if factor_tensor.shape[0] < base.shape[0] or factor_tensor.shape[1] < base.shape[1]:
            return base, {
                "selected_determinant_modifiers": [],
    ...
    for row, scale, prefix in [(item, 1.0, "main") for item in promoted] + [(item, 0.55, "support") for item in supporting]:
        ...
        hooks = list(row.get("transition_hooks") or []) or _default_factor_transition_hooks(row)
...
def _build_representation_modifier_covariates(
    *,
    ...
    elif resolved == "hybrid_multiscale":
        covariates, meta = _build_hybrid_modifier_covariates(
            run_dir=run_dir,
            observation_targets=observation_targets,
            month_axis=month_axis,
            ...
            canonical_axis=canonical_axis,
            candidate_profiles=candidate_profiles,
            mesoscopic_selection_variant="phase2_multiscale",
        )
    elif resolved == "hybrid_temporal_multiscale":
        covariates, meta = _build_hybrid_modifier_covariates(
            run_dir=run_dir,
            observation_targets=observation_targets,
            month_axis=month_axis,
    ...
    elif resolved == "clumped_multiscale":
        covariates, meta = _build_mesoscopic_modifier_covariates(
            run_dir=run_dir,
            observation_targets=observation_targets,
            month_axis=month_axis,
            selection_variant="phase2_multiscale",
        )
    else:
        covariates, meta = _build_mesoscopic_modifier_covariates(
            run_dir=run_dir,
    ...
    covariates, meta = _apply_scaffold_dominance_gate(
        run_dir=run_dir,
...
def _determinant_priority_score(row: dict[str, Any]) -> float:
    if "diagnostic_score" in row:
        return float(row.get("diagnostic_score", 0.0) or 0.0)
    curation_score = float(row.get("curation_score", 0.0) or 0.0)
    dag_score = float(row.get("dag_score", 0.0) or 0.0)
    numeric_support = float(row.get("numeric_support", 0.0) or 0.0)
    promotion_class = str(row.get("promotion_class") or "")
    promotion_bonus = 0.1 if promotion_class == "survivor_primary" else (0.03 if promotion_class == "survivor_secondary" else 0.0)
    return curation_score + dag_score + 0.01 * numeric_support + promotion_bonus


def _scaffold_dominance_summary(run_dir: Any, representation_mode: str) -> dict[str, Any]:
    cfg = dict(_phase3_prior("scaffold_dominance") or {})
    summary: dict[str, Any] = {
        "enabled": bool(cfg.get("enabled", False)),
        "representation_mode": str(representation_mode or ""),
        "fallback_mode": str(cfg.get("fallback_mode") or "keep_as_is"),
        "artifact_path": "",
        "artifact_available": False,
        "selection_row_count": 0,
        "mean_model_mae": None,
        "mean_simple_compartmental_mae": None,
        "mean_carry_forward_mae": None,
        "min_relative_improvement_vs_scaffold": round(float(cfg.get("min_relative_improvement_vs_scaffold", 0.0) or 0.0), 6),
        "passes_scaffold_gate": True,
        "verdict": "not_applied",
    }
    if not summary["enabled"]:
        return summary
    artifact_rel = str(cfg.get("evaluation_artifact") or "phase3_frozen_backtest_tournament/representation_tournament.json")
    artifact_path = Path(run_dir) / artifact_rel
    summary["artifact_path"] = str(artifact_path)
    rows = load_broad_representation_trial_rows(Path(run_dir), representation_mode)
    if rows:
        summary["artifact_available"] = True
        summary["source_kind"] = BROAD_FROZEN_HISTORY_REGIME
        summary["artifact_path"] = str(broad_frozen_tournament_artifact_path(Path(run_dir)))
    if not rows:
        summary["passes_scaffold_gate"] = False
        summary["verdict"] = "artifact_missing"
        return summary
    summary["selection_row_count"] = len(rows)
    model_mae = float(np.mean([float(row.get("model_mean_absolute_error", 0.0) or 0.0) for row in rows]))
    scaffold_mae = float(np.mean([float(row.get("simple_compartmental_mean_absolute_error", 0.0) or 0.0) for row in rows]))
    carry_mae = float(np.mean([float(row.get("carry_forward_mean_absolute_error", 0.0) or 0.0) for row in rows]))
    margin = float(cfg.get("min_relative_improvement_vs_scaffold", 0.0) or 0.0)
    threshold = scaffold_mae * max(0.0, 1.0 - margin)
    passes = model_mae <= threshold
    summary.update(
        {
            "mean_model_mae": round(model_mae, 6),
            "mean_simple_compartmental_mae": round(scaffold_mae, 6),
            "mean_carry_forward_mae": round(carry_mae, 6),
            "required_model_mae_threshold": round(threshold, 6),
            "passes_scaffold_gate": bool(passes),
            "verdict": "passed" if passes else "failed_vs_scaffold",
        }
    )
    return summary


def _apply_scaffold_dominance_gate(
    *,
    run_dir: Any,
    covariates: np.ndarray,
    covariate_meta: dict[str, Any],
    ...
    cfg = dict(_phase3_prior("scaffold_dominance") or {})
    meta = dict(covariate_meta)
    representation_mode = str(meta.get("representation_mode") or "")
    summary = _scaffold_dominance_summary(run_dir, representation_mode)
    selected_rows = [dict(row) for row in list(meta.get("selected_determinant_modifiers") or [])]
    covariate_names = [str(name) for name in list(meta.get("covariate_names") or [])]
    hook_masks = [list(row) for row in list(meta.get("transition_hook_masks") or [])]
    observation_count = 3 if bool(cfg.get("always_keep_observation_covariates", True)) else 0
    if not bool(cfg.get("enabled", False)) or len(covariate_names) <= observation_count or not selected_rows:
        meta["scaffold_dominance_summary"] = summary
        return np.asarray(covariates, dtype=np.float32), meta

    if not bool(summary.get("passes_scaffold_gate")):
        fallback_mode = str(cfg.get("fallback_mode") or "keep_as_is")
        if fallback_mode == "scaffold_only":
            keep_indices = list(range(min(observation_count, covariates.shape[2])))
            trimmed = np.asarray(covariates[:, :, keep_indices], dtype=np.float32)
            meta.update(
                {
                    "selected_determinant_modifiers": [],
                    "covariate_names": covariate_names[: len(keep_indices)],
                    "transition_hook_masks": hook_masks[: len(keep_indices)],
                    "coupling_covariate_names": [],
                    "network_family_indices": {},
                }
            )
            summary["retained_determinant_count"] = 0
            summary["verdict"] = "scaffold_only_enforced"
            meta["scaffold_dominance_summary"] = summary
            return trimmed, meta
        meta["scaffold_dominance_summary"] = summary
        return np.asarray(covariates, dtype=np.float32), meta

    max_retained = max(0, int(cfg.get("max_retained_determinants", len(selected_rows)) or 0))
    if max_retained and len(selected_rows) > max_retained:
        ranked = sorted(
            range(len(selected_rows)),
            key=lambda idx: (_determinant_priority_score(selected_rows[idx]), -idx),
            reverse=True,
        )
        kept_local = sorted(ranked[:max_retained])
        keep_indices = list(range(min(observation_count, covariates.shape[2]))) + [
            observation_count + idx
            for idx in kept_local
            if observation_count + idx < covariates.shape[2]
        ]
        trimmed = np.asarray(covariates[:, :, keep_indices], dtype=np.float32)
        trimmed_names = [covariate_names[idx] for idx in keep_indices]
        trimmed_masks = [hook_masks[idx] for idx in keep_indices]
        trimmed_selected = [selected_rows[idx] for idx in kept_local]
        retained_names = {trimmed_names[observation_count + idx] for idx in range(len(trimmed_selected)) if observation_count + idx < len(trimmed_names)}
        coupling_names = [name for name in list(meta.get("coupling_covariate_names") or []) if name in retained_names]
        network_family_indices: dict[str, list[int]] = defaultdict(list)
        for local_idx, row in enumerate(trimmed_selected, start=observation_count):
            family = str(row.get("network_feature_family") or "")
            if family:
                network_family_indices[family].append(local_idx)
        meta.update(
            {
                "selected_determinant_modifiers": trimmed_selected,
                "covariate_names": trimmed_names,
                "transition_hook_masks": trimmed_masks,
                "coupling_covariate_names": coupling_names,
                "network_family_indices": {key: value for key, value in network_family_indices.items()},
            }
        )
        summary["retained_determinant_count"] = len(trimmed_selected)
        summary["verdict"] = "passed_topk_trimmed"
        meta["scaffold_dominance_summary"] = summary
        return trimmed, meta

    summary["retained_determinant_count"] = len(selected_rows)
    meta["scaffold_dominance_summary"] = summary
    return np.asarray(covariates, dtype=np.float32), meta


def _prepare_temporal_modifier_inputs(
    *,
    covariates: np.ndarray,
    covariate_meta: dict[str, Any],
    month_axis: list[str],
    transition_prior_override: np.ndarray,
) -> tuple[np.ndarray, dict[str, Any], dict[str, Any]]:
    temporal_cfg = dict(_phase3_prior("temporal_decomposition") or {})
    shock_cfg = dict(_phase3_prior("shock_regimes") or {})
    evolution_cfg = _phase3_frozen_backtest_section("evolution")
    temporal_cfg = {
        **temporal_cfg,
        "probability_eps": float(_phase3_stabilizer("probability_eps")),
    ...
    scaffold_points = [dict(point) for point in HARP_PROGRAM_POINTS]
    if len(scaffold_points) < 2:
        scaffold_points = []
        for point in OFFICIAL_REFERENCE_POINTS:
            reference = dict(point.get("reference") or {})
            first95 = float(reference.get("first95", 0.0) or 0.0)
            second95 = float(reference.get("second95", 0.0) or 0.0)
            overall = reference.get("overall_suppressed")
            third95 = reference.get("documented_suppression_among_art")
            on_art = first95 * second95
            suppressed = float(overall) if overall is not None else on_art * float(third95 or 0.0)
            scaffold_points.append(
                {
                    "month": str(point.get("month") or ""),
                    "year": int(point.get("year") or (_month_year(str(point.get("month") or "")) or 0)),
                    "estimated_plhiv": 1.0,
                    "diagnosed": first95,
                    "on_art": on_art,
                    "viral_load_tested": max(suppressed, on_art * 0.75),
                    "suppressed": suppressed,
                }
            )
    temporal_bundle = temporal_representation_bundle(
        train_harp_points=scaffold_points,
        month_axis=month_axis,
        covariates=np.asarray(covariates, dtype=np.float32),
        covariate_meta=dict(covariate_meta),
        transition_prior=transition_prior_override,
        temporal_cfg=temporal_cfg,
        shock_cfg=shock_cfg,
    )
    updated_meta = dict(covariate_meta)
    updated_meta["temporal_projection_summary"] = dict(temporal_bundle.get("projection_summary", {}) or {})
    updated_meta["determinant_group_catalog"] = dict(temporal_bundle.get("group_catalog", {}) or {})
    updated_meta["scaffold_source"] = str(dict(temporal_bundle.get("scaffold", {}) or {}).get("source") or "")
    return np.asarray(temporal_bundle["projected_covariates"], dtype=np.float32), updated_meta, temporal_bundle
...
def _fit_rescue_core_numpy(
    *,
    ...
    mixed_frequency_bundle: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    transition_prior_override = np.asarray((calibration_overrides or {}).get("transition_prior_override", TRANSITION_PRIOR), dtype=np.float32)
    covariates, covariate_meta, temporal_bundle = _prepare_temporal_modifier_inputs(
        covariates=covariates,
        covariate_meta=covariate_meta,
        month_axis=month_axis,
        transition_prior_override=transition_prior_override,
    )
    ...
    base_transition = np.broadcast_to(
        np.asarray(temporal_bundle.get("scaffold", {}).get("monthly_probs", transition_prior_override.reshape(1, -1)), dtype=np.float32).reshape(1, len(month_axis), len(TRANSITION_NAMES)),
        (state_estimates.shape[0], len(month_axis), len(TRANSITION_NAMES)),
    ).astype(np.float32)
    ...
    return {
        ...
        "prediction_stack": {
            ...
            "deaths": _predict_deaths_np(state_estimates[..., 4]),
        },
        "loss_breakdown": {
        ...
        "loss_trace": [],
        "parameters_summary": {
            "national_transition": np.asarray(temporal_bundle.get("scaffold", {}).get("monthly_probs", transition_prior_override.reshape(1, -1)), dtype=np.float32).mean(axis=0).round(6).tolist(),
            ...
            "temporal_hazard_components": {
                "scaffold_source": str(dict(temporal_bundle.get("scaffold", {}) or {}).get("source") or ""),
                "annual_rate_rows": list(dict(temporal_bundle.get("scaffold", {}) or {}).get("annual_rate_rows", []) or []),
                "projection_summary": dict(temporal_bundle.get("projection_summary", {}) or {}),
                "config_summary": dict(temporal_bundle.get("config_summary", {}) or {}),
            },
            "mixed_frequency_observation": dict(mixed_frequency_bundle or {}),
        },
        "device": "cpu",
...
def _rescue_svi_model(
    ...
    diagnosis_flow_start_index: Any,
    diagnosis_flow_end_index: Any,
    diagnosis_flow_target: Any,
    diagnosis_flow_weight: Any,
    ...
    slow_basis: Any,
    medium_basis: Any,
    shock_regime_basis: Any,
    shock_transition_mask: Any,
    year_index_basis: Any,
    year_count: int,
    determinant_bound_scale: float,
    ...
    temporal_cfg = dict(_phase3_prior("temporal_decomposition") or {})
    jax_loss_cfg = dict(_phase3_prior("jax_svi_loss_scales") or {})
    ...
    slow_basis_count = int(slow_basis.shape[-1])
    medium_basis_count = int(medium_basis.shape[-1])
    shock_basis_count = int(shock_regime_basis.shape[-1]) if shock_regime_basis.ndim == 2 else 0
    scaffold_center = jnp.mean(scaffold_logit, axis=0)
    scaffold_delta = scaffold_logit - scaffold_center.reshape(1, len(TRANSITION_NAMES))
    prior_logit = scaffold_center
    national_logit = numpyro.sample("national_logit", dist.Normal(prior_logit, float(svi_cfg["transition_prior_sigma"])).to_event(1))
    slow_temporal_coef = (
        numpyro.sample(
            "slow_temporal_coef",
            dist.Normal(
                jnp.zeros((slow_basis_count, len(TRANSITION_NAMES)), dtype=jnp.float32),
                float(temporal_cfg["slow_sigma"]),
            ).to_event(2),
        )
        if slow_basis_count > 0
        else jnp.zeros((0, len(TRANSITION_NAMES)), dtype=jnp.float32)
    )
    if slow_basis_count == 0:
        slow_temporal_coef = numpyro.deterministic("slow_temporal_coef", slow_temporal_coef)
    medium_temporal_coef = (
        numpyro.sample(
            "medium_temporal_coef",
            dist.Normal(
                jnp.zeros((medium_basis_count, len(TRANSITION_NAMES)), dtype=jnp.float32),
                float(temporal_cfg["medium_sigma"]),
            ).to_event(2),
        )
        if medium_basis_count > 0
        else jnp.zeros((0, len(TRANSITION_NAMES)), dtype=jnp.float32)
    )
    if medium_basis_count == 0:
        medium_temporal_coef = numpyro.deterministic("medium_temporal_coef", medium_temporal_coef)
    shock_regime_offset = (
        numpyro.sample(
            "shock_regime_offset",
            dist.Normal(
                jnp.zeros((shock_basis_count, len(TRANSITION_NAMES)), dtype=jnp.float32),
                float(temporal_cfg["shock_sigma"]),
            ).to_event(2),
        )
        if shock_basis_count > 0
        else jnp.zeros((0, len(TRANSITION_NAMES)), dtype=jnp.float32)
    )
    if shock_basis_count == 0:
        shock_regime_offset = numpyro.deterministic("shock_regime_offset", shock_regime_offset)
    region_year_offset = numpyro.sample(
        "region_year_offset",
        dist.Normal(
            jnp.zeros((region_count, year_count, len(TRANSITION_NAMES)), dtype=jnp.float32),
            float(temporal_cfg["region_year_sigma"]),
        ).to_event(3),
    )
    region_offset = numpyro.deterministic("region_offset", jnp.mean(region_year_offset, axis=1))
    ...
    slow_effect = (
        jnp.einsum("ms,sr->mr", slow_basis, slow_temporal_coef)
        if slow_basis_count > 0
        else jnp.zeros((month_count, len(TRANSITION_NAMES)), dtype=jnp.float32)
    )
    medium_effect = (
        jnp.einsum("ms,sr->mr", medium_basis, medium_temporal_coef)
        if medium_basis_count > 0
        else jnp.zeros((month_count, len(TRANSITION_NAMES)), dtype=jnp.float32)
    )
    shock_effect = (
        jnp.einsum("ms,sr->mr", shock_regime_basis, shock_regime_offset * shock_transition_mask)
        if shock_basis_count > 0
        else jnp.zeros((month_count, len(TRANSITION_NAMES)), dtype=jnp.float32)
    )
    temporal_component = scaffold_delta + slow_effect + medium_effect + shock_effect
    bounded_hook = determinant_bound_scale * jnp.tanh(jnp.einsum("ptc,cr->ptr", covariates, covariate_weights * hook_mask))
    logits = (
        national_logit.reshape(1, 1, len(TRANSITION_NAMES))
        + temporal_component.reshape(1, month_count, len(TRANSITION_NAMES))
        + region_year_offset[region_index][:, year_index_basis, :]
    ...
    numpyro.deterministic("slow_temporal_component", slow_effect)
    numpyro.deterministic("medium_temporal_component", medium_effect)
    numpyro.deterministic("shock_temporal_component", shock_effect)
    ...
    if int(diagnosis_flow_target.shape[0]) > 0:
        national_u_to_d = jnp.sum(state_path[..., 0] * probs[..., 0] * national_mask.reshape(province_count, 1), axis=0)
        diagnosis_flow_pred = _diagnosis_flow_segment_sums_jax(
            national_u_to_d,
            diagnosis_flow_start_index,
            diagnosis_flow_end_index,
        )
        diagnosis_flow_penalty = jnp.mean(diagnosis_flow_weight * jnp.square(diagnosis_flow_pred - diagnosis_flow_target))
        numpyro.factor("diagnosis_flow_penalty", -float(jax_loss_cfg["diagnosis_flow_penalty_scale"]) * diagnosis_flow_penalty)

    sigma_diag = numpyro.sample("sigma_diag", dist.HalfNormal(float(svi_cfg["sigma_diag_scale"])))
    sigma_art = numpyro.sample("sigma_art", dist.HalfNormal(float(svi_cfg["sigma_art_scale"])))
    sigma_sup = numpyro.sample("sigma_sup", dist.HalfNormal(float(svi_cfg["sigma_sup_scale"])))
    sigma_test = numpyro.sample("sigma_test", dist.HalfNormal(float(svi_cfg["sigma_test_scale"])))
    ...
    lower_bound = jnp.maximum(
        latent_sup + float(init_cfg["suppression_margin"]),
        latent_test * float(init_cfg["testing_ceiling_scale"]),
    )
    numpyro.factor(
        "suppression_lower_bound_penalty",
        -float(jax_loss_cfg["suppression_lower_bound_penalty"]) * jnp.mean(jnp.square(jax.nn.relu(suppression_pred - lower_bound))),
    )
    ...
    numpyro.factor("national_anchor_penalty", -float(jax_loss_cfg["national_anchor_penalty_scale"]) * (anchor_penalty + jnp.mean(third_penalty)))
    ...
    numpyro.factor(
        ...
        -float(jax_loss_cfg["harp_program_penalty_scale"])
        * (
            harp_penalty
            + jnp.mean(harp_ratio_penalty)
            + jnp.mean(harp_sup_ratio_penalty)
            + float(jax_loss_cfg["harp_vl_smooth_weight"]) * vl_smooth_penalty
        ),
    )
    linkage_penalty = jnp.mean(
        linkage_weight
        * (
            jnp.square(national_d_to_a - linkage_d_to_a_target)
            + float(jax_loss_cfg["linkage_second95_weight"])
            * jnp.square(national_art / jnp.clip(national_diag, 1e-6, None) - linkage_second95_target)
        )
    )
    numpyro.factor("linkage_penalty", -float(jax_loss_cfg["linkage_penalty_scale"]) * linkage_penalty)
    ...
    numpyro.factor("suppression_penalty", -float(jax_loss_cfg["suppression_penalty_scale"]) * suppression_penalty)
    numpyro.factor(
        "synthetic_pretraining_penalty",
        -float(jax_loss_cfg["synthetic_pretraining_penalty_scale"])
        * (
            ...
            + float(jax_loss_cfg["synthetic_transition_anchor_weight"]) * jnp.mean(jnp.square(archetype_transition_scale - 1.0))
            + float(jax_loss_cfg["synthetic_observation_anchor_weight"]) * jnp.mean(jnp.square(archetype_observation_scale - 1.0))
        ),
    )
...
def _fit_rescue_core_jax_svi(
    *,
    ...
    transition_prior_override = np.asarray((calibration_overrides or {}).get("transition_prior_override", TRANSITION_PRIOR), dtype=np.float32)
    covariates_np, covariate_meta, temporal_bundle = _prepare_temporal_modifier_inputs(
        covariates=np.asarray(covariates_np, dtype=np.float32),
        covariate_meta=covariate_meta,
        month_axis=month_axis,
        transition_prior_override=transition_prior_override,
    )
    ...
    scaffold_logit = numpy_to_jax_handoff(np.asarray(temporal_bundle["scaffold"]["monthly_logits"], dtype=np.float32)).astype(jnp.float32)
    slow_basis = numpy_to_jax_handoff(np.asarray(temporal_bundle["basis"]["slow_basis"], dtype=np.float32)).astype(jnp.float32)
    medium_basis = numpy_to_jax_handoff(np.asarray(temporal_bundle["basis"]["medium_basis"], dtype=np.float32)).astype(jnp.float32)
    year_index_basis = numpy_to_jax_handoff(np.asarray(temporal_bundle["basis"]["year_index"], dtype=np.int32)).astype(jnp.int32)
    year_count = int(len(np.asarray(temporal_bundle["basis"]["year_labels"], dtype=np.int32)))
    shock_regime_basis = numpy_to_jax_handoff(np.asarray(dict(temporal_bundle.get("shock_basis", {}) or {}).get("basis", np.zeros((len(month_axis), 0), dtype=np.float32)), dtype=np.float32)).astype(jnp.float32)
    shock_transition_mask_np = np.ones((shock_regime_basis.shape[-1], len(TRANSITION_NAMES)), dtype=np.float32)
    shock_rows = list(dict(_phase3_prior("shock_regimes") or {}).get("regimes", []) or [])
    if shock_rows:
        for regime_idx, row in enumerate(shock_rows[: shock_transition_mask_np.shape[0]]):
            allowed = {str(name) for name in list(row.get("transition_names") or [])}
            if allowed:
                shock_transition_mask_np[regime_idx] = np.asarray([1.0 if name in allowed else 0.0 for name in TRANSITION_NAMES], dtype=np.float32)
    shock_transition_mask = numpy_to_jax_handoff(shock_transition_mask_np).astype(jnp.float32)
    determinant_bound_scale = float(temporal_cfg["determinant_bound_scale"])
    ...
    diagnosis_flow_arrays = _diagnosis_flow_reference_arrays(month_axis)
    mixed_frequency_bundle = dict(mixed_frequency_bundle or {})
    official_anchor_arrays = dict(mixed_frequency_bundle.get("official_anchor_arrays", {}) or {})
    harp_anchor_arrays = dict(mixed_frequency_bundle.get("harp_anchor_arrays", {}) or {})
    national_anchor_diag = numpy_to_jax_handoff(np.asarray(official_anchor_arrays.get("diagnosed_stock", anchor_curves["diagnosed_stock"]), dtype=np.float32)).astype(jnp.float32)
    national_anchor_art = numpy_to_jax_handoff(np.asarray(official_anchor_arrays.get("art_stock", anchor_curves["art_stock"]), dtype=np.float32)).astype(jnp.float32)
    national_anchor_sup = numpy_to_jax_handoff(np.asarray(official_anchor_arrays.get("documented_suppression", anchor_curves["documented_suppression"]), dtype=np.float32)).astype(jnp.float32)
    national_anchor_third = numpy_to_jax_handoff(np.asarray(official_anchor_arrays.get("third95", anchor_curves["third95"]), dtype=np.float32)).astype(jnp.float32)
    national_anchor_weight = numpy_to_jax_handoff(np.asarray(official_anchor_arrays.get("weight", anchor_curves["weight"]), dtype=np.float32)).astype(jnp.float32)
    ...
    harp_diag = numpy_to_jax_handoff(np.asarray(harp_anchor_arrays.get("diagnosed_stock", harp_curves["diagnosed_stock"]), dtype=np.float32)).astype(jnp.float32)
    harp_art = numpy_to_jax_handoff(np.asarray(harp_anchor_arrays.get("art_stock", harp_curves["art_stock"]), dtype=np.float32)).astype(jnp.float32)
    harp_tested = numpy_to_jax_handoff(np.asarray(harp_anchor_arrays.get("testing_coverage", harp_curves["viral_load_tested_stock"]), dtype=np.float32)).astype(jnp.float32)
    harp_sup = numpy_to_jax_handoff(np.asarray(harp_anchor_arrays.get("documented_suppression", harp_curves["documented_suppression"]), dtype=np.float32)).astype(jnp.float32)
    harp_tested_among_art = numpy_to_jax_handoff(np.asarray(harp_anchor_arrays.get("viral_load_tested_among_art", harp_curves["viral_load_tested_among_art"]), dtype=np.float32)).astype(jnp.float32)
    harp_suppressed_among_art = numpy_to_jax_handoff(np.asarray(harp_anchor_arrays.get("suppressed_among_art", harp_curves["suppressed_among_art"]), dtype=np.float32)).astype(jnp.float32)
    harp_weight = numpy_to_jax_handoff(np.asarray(harp_anchor_arrays.get("weight", harp_curves["weight"]), dtype=np.float32)).astype(jnp.float32)
    ...
    diagnosis_flow_start_index = numpy_to_jax_handoff(np.asarray(diagnosis_flow_arrays["start_index"], dtype=np.int32)).astype(jnp.int32)
    diagnosis_flow_end_index = numpy_to_jax_handoff(np.asarray(diagnosis_flow_arrays["end_index"], dtype=np.int32)).astype(jnp.int32)
    diagnosis_flow_target = numpy_to_jax_handoff(np.asarray(diagnosis_flow_arrays["target_share"], dtype=np.float32)).astype(jnp.float32)
    diagnosis_flow_weight = numpy_to_jax_handoff(np.asarray(diagnosis_flow_arrays["weight"], dtype=np.float32)).astype(jnp.float32)
    ...
    model_kwargs = {
        ...
        "diagnosis_flow_start_index": diagnosis_flow_start_index,
        "diagnosis_flow_end_index": diagnosis_flow_end_index,
        "diagnosis_flow_target": diagnosis_flow_target,
        "diagnosis_flow_weight": diagnosis_flow_weight,
        ...
        "scaffold_logit": scaffold_logit,
        "slow_basis": slow_basis,
        "medium_basis": medium_basis,
        "shock_regime_basis": shock_regime_basis,
        "shock_transition_mask": shock_transition_mask,
        "year_index_basis": year_index_basis,
        "year_count": year_count,
        "determinant_bound_scale": determinant_bound_scale,
    ...
    return_sites = [
        ...
        "medium_temporal_coef",
        "shock_regime_offset",
        "slow_temporal_component",
        "medium_temporal_component",
        "shock_temporal_component",
        "archetype_transition_scale_log",
        "archetype_process_scale_log",
        "archetype_observation_scale_log",
        "horseshoe_global",
    ...
    shock_site_count = int(shock_transition_mask_np.shape[0])
    diagnosis_flow_rows: list[dict[str, Any]] = []
    diagnosis_flow_penalty_value = 0.0
    if int(np.asarray(diagnosis_flow_arrays["target_share"], dtype=np.float32).size) > 0:
        national_flow = np.sum(
            state_path[..., 0] * transition_probs[..., 0] * _national_reference_mask(province_axis).reshape(len(province_axis), 1),
            axis=0,
        ).astype(np.float32)
        predicted_flow = np.asarray(
            [
                float(np.sum(national_flow[int(start_idx) : int(end_idx) + 1]))
                for start_idx, end_idx in zip(
                    np.asarray(diagnosis_flow_arrays["start_index"], dtype=np.int32),
                    np.asarray(diagnosis_flow_arrays["end_index"], dtype=np.int32),
                    strict=True,
                )
            ],
            dtype=np.float32,
        )
        target_flow = np.asarray(diagnosis_flow_arrays["target_share"], dtype=np.float32)
        weight_flow = np.asarray(diagnosis_flow_arrays["weight"], dtype=np.float32)
        diagnosis_flow_penalty_value = float(np.mean(weight_flow * np.square(predicted_flow - target_flow))) if predicted_flow.size else 0.0
        diagnosis_flow_rows = [
            {
                **dict(row),
                "predicted_diagnosed_share": round(float(predicted_flow[idx]), 6),
                "absolute_error": round(float(abs(predicted_flow[idx] - target_flow[idx])), 6),
            }
            for idx, row in enumerate(list(diagnosis_flow_arrays.get("rows", []) or []))
        ]
    return {
        ...
        "prediction_stack": {
            ...
            "deaths": _predict_deaths_np(state_path[..., 4]),
        },
        "loss_breakdown": {
            ...
            "diagnosis_flow_penalty": diagnosis_flow_penalty_value,
            "linkage_penalty": 0.0,
        ...
        "parameters_summary": {
            ...
            "diagnosis_flow_summary": {
                "point_count": int(len(diagnosis_flow_rows)),
            ...
            "intervention_tensor": intervention_summary,
            "temporal_hazard_components": {
                "scaffold_source": temporal_bundle["scaffold"]["source"],
                "annual_rate_rows": list(temporal_bundle["scaffold"]["annual_rate_rows"]),
                "projection_summary": dict(temporal_bundle.get("projection_summary", {}) or {}),
                "slow_basis_shape": list(np.asarray(temporal_bundle["basis"]["slow_basis"]).shape),
                "medium_basis_shape": list(np.asarray(temporal_bundle["basis"]["medium_basis"]).shape),
                "national_slow_coefficients": np.round(to_numpy(samples["slow_temporal_coef"]).mean(axis=0), 6).tolist(),
                "national_medium_coefficients": np.round(to_numpy(samples["medium_temporal_coef"]).mean(axis=0), 6).tolist(),
                "region_year_offset_sd": round(float(np.std(to_numpy(samples["region_year_offset"]))), 6),
            },
            "shock_regime_summary": {
                "enabled": bool(shock_site_count),
                "regime_names": list(dict(temporal_bundle.get("shock_basis", {}) or {}).get("names", [])),
                "transition_mask": np.round(shock_transition_mask_np, 6).tolist(),
                "coefficients": np.round(to_numpy(samples["shock_regime_offset"]).mean(axis=0), 6).tolist() if shock_site_count > 0 else [],
            },
...
def _fit_rescue_core_torch(
    *,
    ...
    diagnosis_flow_penalty_scale = float(calibration_overrides.get("diagnosis_flow_penalty_scale", loss_scale_cfg["diagnosis_flow_penalty_scale"]))
    ...
    scaffold_logits_np = np.asarray(temporal_bundle["scaffold"]["monthly_logits"], dtype=np.float32)
    slow_basis_np = np.asarray(temporal_bundle["basis"]["slow_basis"], dtype=np.float32)
    medium_basis_np = np.asarray(temporal_bundle["basis"]["medium_basis"], dtype=np.float32)
    year_index_np = np.asarray(temporal_bundle["basis"]["year_index"], dtype=np.int64)
    year_labels_np = np.asarray(temporal_bundle["basis"]["year_labels"], dtype=np.int64)
    shock_basis_np = np.asarray(dict(temporal_bundle.get("shock_basis", {}) or {}).get("basis", np.zeros((month_count, 0), dtype=np.float32)), dtype=np.float32)
    shock_transition_mask_np = np.ones((shock_basis_np.shape[-1], len(TRANSITION_NAMES)), dtype=np.float32)
    shock_rows = [dict(row) for row in list(dict(_phase3_prior("shock_regimes") or {}).get("regimes", []) or [])]
    if shock_rows:
        for regime_idx, row in enumerate(shock_rows[: shock_transition_mask_np.shape[0]]):
            allowed = {str(name) for name in list(row.get("transition_names") or [])}
            if allowed:
    ...
    scaffold_logit_t = to_torch_tensor(scaffold_logits_np, device=device, dtype=torch.float32)
    scaffold_center_t = scaffold_logit_t.mean(dim=0)
    scaffold_delta_t = scaffold_logit_t - scaffold_center_t.view(1, len(TRANSITION_NAMES))
    slow_basis_t = to_torch_tensor(slow_basis_np, device=device, dtype=torch.float32)
    medium_basis_t = to_torch_tensor(medium_basis_np, device=device, dtype=torch.float32)
    year_index_t = to_torch_tensor(year_index_np, device=device, dtype=torch.long)
    shock_basis_t = to_torch_tensor(shock_basis_np, device=device, dtype=torch.float32)
    shock_transition_mask_t = to_torch_tensor(shock_transition_mask_np, device=device, dtype=torch.float32)
    determinant_bound_scale = float(temporal_cfg["determinant_bound_scale"])

    region_count = int(region_index.max()) + 1 if region_index.size else 1
    transition_prior_logit = scaffold_center_t.detach().cpu().numpy().astype(np.float32)
    ...
    slow_transition_coef = torch.nn.Parameter(torch.zeros((slow_basis_np.shape[-1], len(TRANSITION_NAMES)), device=device, dtype=torch.float32))
    medium_transition_coef = torch.nn.Parameter(torch.zeros((medium_basis_np.shape[-1], len(TRANSITION_NAMES)), device=device, dtype=torch.float32))
    region_year_transition = torch.nn.Parameter(torch.zeros((region_count, max(int(year_labels_np.size), 1), len(TRANSITION_NAMES)), device=device, dtype=torch.float32))
    shock_transition_coef = torch.nn.Parameter(torch.zeros((shock_basis_np.shape[-1], len(TRANSITION_NAMES)), device=device, dtype=torch.float32))
    ...
    parameters = [
        national_transition,
        slow_transition_coef,
        medium_transition_coef,
        region_year_transition,
        shock_transition_coef,
        province_transition,
        kp_transition,
        age_transition,
        sex_transition,
    ...
    def _forward() -> tuple[Any, Any, Any, dict[str, float], dict[str, Any], dict[str, Any], dict[str, Any]]:
        ...
        slow_effect_t = slow_basis_t @ slow_transition_coef if slow_basis_t.numel() else torch.zeros((month_count, len(TRANSITION_NAMES)), device=device, dtype=torch.float32)
        medium_effect_t = medium_basis_t @ medium_transition_coef if medium_basis_t.numel() else torch.zeros((month_count, len(TRANSITION_NAMES)), device=device, dtype=torch.float32)
        shock_effect_t = (
            shock_basis_t @ (shock_transition_coef * shock_transition_mask_t)
            if shock_basis_t.numel()
            else torch.zeros((month_count, len(TRANSITION_NAMES)), device=device, dtype=torch.float32)
        )
        temporal_transition_logits = scaffold_delta_t + slow_effect_t + medium_effect_t + shock_effect_t
        region_year_effect_t = region_year_transition[region_index_t][:, year_index_t, :]
        transition_logits = (
            national_transition.view(1, 1, 1, 1, 1, 1, len(TRANSITION_NAMES))
            + temporal_transition_logits.view(1, 1, 1, 1, 1, month_count, len(TRANSITION_NAMES))
            + region_year_effect_t.view(province_count, 1, 1, 1, 1, month_count, len(TRANSITION_NAMES))
        ...
        covariate_effect = determinant_bound_scale * torch.tanh(torch.einsum("ptc,cr->ptr", covariates_t, covariate_coef * hook_mask_t))
        ...
        if coupling_indices:
            coupling_pressure = float(init_cfg["coupling_pressure_scale"]) * torch.sigmoid(torch.mean(covariates_t[..., coupling_indices], dim=-1))
        else:
        ...
        deaths_pred = _predict_deaths_torch(aggregate[..., 4])
        ...
        national_anchor_penalty, national_anchor_detail = _national_anchor_penalty_torch(
            ...
            month_axis=month_axis,
            device=device,
            curve_overrides=official_anchor_arrays,
        )
        harp_program_penalty, harp_program_detail = _harp_program_penalty_torch(
            ...
            month_axis=month_axis,
            device=device,
            curve_overrides=harp_anchor_arrays,
        )
        diagnosis_flow_penalty, diagnosis_flow_detail, diagnosis_flow_summary = _diagnosis_flow_penalty_torch(
            latent=latent,
        ...
        temporal_smooth = (
            (torch.mean((slow_transition_coef[1:] - slow_transition_coef[:-1]) ** 2) if slow_transition_coef.shape[0] > 1 else torch.tensor(0.0, device=device, dtype=torch.float32))
            + (torch.mean((medium_transition_coef[1:] - medium_transition_coef[:-1]) ** 2) if medium_transition_coef.shape[0] > 1 else torch.tensor(0.0, device=device, dtype=torch.float32))
        )
        transition_reg = (
            float(reg_cfg["region_transition"]) * torch.mean(region_year_transition**2)
            + float(reg_cfg["province_transition_prior"]) * torch.mean((province_transition - scaled_archetype_transition_prior) ** 2)
            + float(temporal_cfg["temporal_regularization"])
            * (
                torch.mean(slow_transition_coef**2)
                + torch.mean(medium_transition_coef**2)
                + torch.mean(shock_transition_coef**2)
                + temporal_smooth
            )
        )
        ...
        plausibility_penalty = float(reg_cfg["plausibility"]) * torch.mean((torch.sigmoid(national_transition) - torch.sigmoid(scaffold_center_t)) ** 2)
        ...
        total_loss = (
            ...
            + diagnosis_flow_penalty_scale * diagnosis_flow_penalty
        ...
        breakdown = {
            ...
            "diagnosis_flow_penalty": float(diagnosis_flow_penalty.detach().cpu()),
            ...
            "temporal_component_penalty": float(transition_reg.detach().cpu()),
        ...
        breakdown.update(diagnosis_flow_detail)
        ...
        aux_summary = {
            ...
            "diagnosis_flow_summary": {
                "point_count": int(diagnosis_flow_detail.get("diagnosis_flow_point_count", 0)),
                "rows": list(diagnosis_flow_summary.get("rows", []) or []),
            },
        }
    ...
    with torch.no_grad():
        ...
        parameters_summary = {
            ...
            "region_transition_sd": round(float(region_year_transition.detach().std().cpu()), 6),
            ...
            "calibration_overrides": {
                ...
                "diagnosis_flow_penalty_scale": diagnosis_flow_penalty_scale,
            ...
            "temporal_hazard_components": {
                ...
                "annual_rate_rows": list(temporal_bundle["scaffold"]["annual_rate_rows"]),
                "projection_summary": dict(temporal_bundle.get("projection_summary", {}) or {}),
                "slow_basis_shape": list(slow_basis_np.shape),
                "medium_basis_shape": list(medium_basis_np.shape),
                "national_slow_coefficients": to_numpy(slow_transition_coef).round(6).tolist(),
                "national_medium_coefficients": to_numpy(medium_transition_coef).round(6).tolist(),
                "region_year_offset_sd": round(float(region_year_transition.detach().std().cpu()), 6),
            },
            "shock_regime_summary": {
                "enabled": bool(shock_basis_np.shape[-1]),
                "regime_names": list(dict(temporal_bundle.get("shock_basis", {}) or {}).get("names", [])),
                "transition_mask": shock_transition_mask_np.round(6).tolist(),
                "coefficients": to_numpy(shock_transition_coef).round(6).tolist(),
            },
            "metapopulation_engine": {
                "enabled": bool(operator_names),
                "operator_names": operator_names,
        ...
        return {
            ...
            "prediction_stack": {
                ...
                "deaths": _predict_deaths_np(aggregate_np[..., 4]),
            },
...
def _forecast_prediction_stack(
    ...
    return {
        ...
        "documented_suppression": documented_suppression,
        "deaths": _predict_deaths_np(forecast_states[..., 4]),
    }
...
def _posterior_prediction_stack_from_samples(samples: dict[str, Any]) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "testing_coverage": testing_coverage,
        "documented_suppression": documented_suppression,
        "deaths": _predict_deaths_np(state_path_samples[..., 4]),
    }
...
def _official_reference_penalty_torch(
    *,
    ...
    for point in OFFICIAL_REFERENCE_POINTS:
        resolved_month, _ = _resolved_point_month(point, month_axis)
        if resolved_month not in month_axis:
            continue
        month_idx = month_axis.index(str(resolved_month))
        ...
        if "first95" in point["reference"]:
            ...
            details[f"{resolved_month}_first95_error"] = float(torch.abs(diag_val - float(point["reference"]["first95"])).detach().cpu())
        if "second95" in point["reference"]:
            penalty = penalty + (second_val - float(point["reference"]["second95"])) ** 2
            details[f"{resolved_month}_second95_error"] = float(torch.abs(second_val - float(point["reference"]["second95"])).detach().cpu())
        if "overall_suppressed" in point["reference"]:
            penalty = penalty + (sup_val - float(point["reference"]["overall_suppressed"])) ** 2
            details[f"{resolved_month}_overall_error"] = float(torch.abs(sup_val - float(point["reference"]["overall_suppressed"])).detach().cpu())
        if "documented_suppression_among_art" in point["reference"]:
            ...
            details[f"{resolved_month}_third95_error"] = float(torch.abs(third_val - float(point["reference"]["documented_suppression_among_art"])).detach().cpu())
...
def _national_anchor_penalty_torch(
    *,
    ...
    curves = dict(curve_overrides or {})
    if not curves:
        curves = _official_anchor_curves(month_axis)
    weight_array = np.asarray(curves["weight"], dtype=np.float32)
    if not np.any(weight_array > 0):
        zero = torch.tensor(0.0, device=device, dtype=torch.float32)
        return zero, {"national_anchor_penalty": 0.0}
    mask = to_torch_tensor(mask_np, device=device, dtype=torch.float32)
    weights = to_torch_tensor(weight_array, device=device, dtype=torch.float32)
    diag_curve = to_torch_tensor(np.asarray(curves["diagnosed_stock"], dtype=np.float32), device=device, dtype=torch.float32)
    art_curve = to_torch_tensor(np.asarray(curves["art_stock"], dtype=np.float32), device=device, dtype=torch.float32)
    sup_curve = to_torch_tensor(np.asarray(curves["documented_suppression"], dtype=np.float32), device=device, dtype=torch.float32)
    third_curve = to_torch_tensor(np.asarray(curves["third95"], dtype=np.float32), device=device, dtype=torch.float32)
...
def _harp_program_penalty_torch(
    *,
    ...
    diag_curve = to_torch_tensor(np.asarray(curves["diagnosed_stock"], dtype=np.float32), device=device, dtype=torch.float32)
    art_curve = to_torch_tensor(np.asarray(curves["art_stock"], dtype=np.float32), device=device, dtype=torch.float32)
    tested_curve = to_torch_tensor(np.asarray(curves.get("viral_load_tested_stock", curves.get("testing_coverage", np.zeros((len(month_axis),), dtype=np.float32))), dtype=np.float32), device=device, dtype=torch.float32)
    sup_curve = to_torch_tensor(np.asarray(curves["documented_suppression"], dtype=np.float32), device=device, dtype=torch.float32)
    tested_among_art_curve = to_torch_tensor(np.asarray(curves["viral_load_tested_among_art"], dtype=np.float32), device=device, dtype=torch.float32)
    suppressed_among_art_curve = to_torch_tensor(np.asarray(curves["suppressed_among_art"], dtype=np.float32), device=device, dtype=torch.float32)
...
def _diagnosis_flow_penalty_torch(
    *,
    latent: Any,
    ...
    mask_np = _national_reference_mask(province_axis)
    arrays = _diagnosis_flow_reference_arrays(month_axis)
    if not np.any(mask_np) or int(arrays["target_share"].size) == 0:
        zero = torch.tensor(0.0, device=device, dtype=torch.float32)
        return zero, {"diagnosis_flow_penalty": 0.0, "diagnosis_flow_point_count": 0}, arrays

    mask = to_torch_tensor(mask_np, device=device, dtype=torch.float32)
    weights = to_torch_tensor(np.asarray(arrays["weight"], dtype=np.float32), device=device, dtype=torch.float32)
    target_share = to_torch_tensor(np.asarray(arrays["target_share"], dtype=np.float32), device=device, dtype=torch.float32)
    start_index = [int(value) for value in np.asarray(arrays["start_index"], dtype=np.int32).tolist()]
    end_index = [int(value) for value in np.asarray(arrays["end_index"], dtype=np.int32).tolist()]

    undiagnosed_mass = latent[..., 0, :, :]
    subgroup_weighted_mass = undiagnosed_mass * subgroup_weights.unsqueeze(-1).unsqueeze(-1)
    u_to_d_prob = transition_probs[..., 0, :]
    province_flow = torch.sum(subgroup_weighted_mass * u_to_d_prob, dim=(1, 2, 3, 4))
    national_flow = torch.sum(province_flow * mask.view(-1, 1), dim=0)
    predicted_share = torch.stack([torch.sum(national_flow[start_idx : end_idx + 1]) for start_idx, end_idx in zip(start_index, end_index, strict=True)])

    penalty = torch.mean(weights * torch.square(predicted_share - target_share))
    details = {
        "diagnosis_flow_penalty": float(penalty.detach().cpu()),
        "diagnosis_flow_mae": float(torch.mean(torch.abs(predicted_share - target_share) * weights).detach().cpu()),
        "diagnosis_flow_point_count": int(len(start_index)),
        "diagnosis_flow_predicted_mean": float(torch.mean(predicted_share).detach().cpu()) if predicted_share.numel() else 0.0,
        "diagnosis_flow_target_mean": float(torch.mean(target_share).detach().cpu()) if target_share.numel() else 0.0,
    }
    rows = []
    for idx, row in enumerate(list(arrays.get("rows") or [])):
        rows.append(
            {
                **dict(row),
                "predicted_diagnosed_share": round(float(predicted_share[idx].detach().cpu()), 6),
                "absolute_error": round(float(torch.abs(predicted_share[idx] - target_share[idx]).detach().cpu()), 6),
            }
        )
    return penalty, details, {**arrays, "rows": rows}
...
def _harp_program_check(
    *,
    ...
    for point in HARP_PROGRAM_POINTS:
        resolved_month, _ = _resolved_point_month(point, month_axis)
        point_ordinal = _month_ordinal(resolved_month or "")
...
def _archive_report_availability_month(source_id: str, source_label: str) -> str | None:
    if not str(source_id or "").startswith("doh_hiv_sti_"):
        return None
    ...
    month_name = str(month_matches[-1]).strip().lower()
    month = _ARCHIVE_REPORT_MONTH_NUMBERS.get(month_name)
    if month is None:
        return None
    return f"{year:04d}-{month:02d}"


def _frozen_vintage_incumbent_points(run_dir: Any, forecast_origin_month: str) -> dict[int, dict[str, Any]]:
    forecast_origin_ordinal = _month_ordinal(str(forecast_origin_month or ""))
    if forecast_origin_ordinal is None:
        return {}
    ...
    for raw_row in metric_rows:
        ...
        source_label = str(raw_row.get("source_label") or "")
        availability_month = _archive_report_availability_month(source_id, source_label)
        availability_ordinal = _month_ordinal(availability_month or "")
        if availability_ordinal is None or availability_ordinal > forecast_origin_ordinal:
            continue
        year = int(raw_row.get("year") or 0)
        metric_name = str(raw_row.get("metric_name") or "")
        if year <= 0 or metric_name not in required_metrics:
            continue
        key = (year, source_id)
        ...
        entry[metric_name] = float(raw_row.get("value") or 0.0)

    selected: dict[int, dict[str, Any]] = {}
    for (year, _source_id), entry in grouped.items():
        if not required_metrics.issubset(entry):
            continue
        previous = selected.get(year)
        previous_ordinal = _month_ordinal(str(previous.get("availability_month") or "")) if previous else None
        entry_ordinal = _month_ordinal(str(entry.get("availability_month") or ""))
        if previous is None or (entry_ordinal is not None and (previous_ordinal is None or entry_ordinal >= previous_ordinal)):
            selected[year] = dict(entry)
    return selected


def _frozen_vintage_holdout_benchmark(
    *,
    run_id: str,
    profile_id: str,
    inference_family: str,
    ...
    if not backtest_config:
        return {"available": False, "reason": "backtest_config_missing", "comparisons": []}
    ...
    forecast_origin_month = str((backtest_config or {}).get("train_last_model_month") or "")
    if not forecast_origin_month:
        return {"available": False, "reason": "forecast_origin_missing", "comparisons": []}
    forecast_month_axis = _forecast_month_axis(
        forecast_origin_month,
        int((backtest_config or {}).get("forecast_horizon") or forecast_states.shape[1] or 0),
    )
    holdout_points = list((backtest_config or {}).get("holdout_harp_points") or [])
    holdout_lookup: dict[int, dict[str, Any]] = {}
    for point in holdout_points:
        ...
        if year <= 0:
            continue
        previous = holdout_lookup.get(year)
        point_ordinal = _month_ordinal(str(point.get("month") or "")) or -1
        previous_ordinal = _month_ordinal(str(previous.get("month") or "")) or -1 if previous else -1
        if previous is None or point_ordinal >= previous_ordinal:
            holdout_lookup[year] = dict(point)

    ctx = RunContext.create(run_id=run_id, plugin_id="hiv")
    incumbent_lookup = _frozen_vintage_incumbent_points(ctx.run_dir, forecast_origin_month)
    comparisons: list[dict[str, Any]] = []
    missing_years: list[int] = []
    for holdout_year in sorted(holdout_lookup):
        holdout_point = holdout_lookup[holdout_year]
        selected_month = str(holdout_point.get("month") or "")
        incumbent = incumbent_lookup.get(holdout_year)
        if incumbent is None:
            missing_years.append(holdout_year)
            continue
        if selected_month not in forecast_month_axis:
            missing_years.append(holdout_year)
            continue
        source = np.tensordot(national_mask, forecast_states[:, forecast_month_axis.index(selected_month)], axes=(0, 0))
        model_metrics_raw = _state_to_cascade_metrics(np.asarray(source, dtype=np.float32))
        model_metrics = {
            "diagnosed_stock": float(model_metrics_raw["diagnosed_share"]),
            "art_stock": float(model_metrics_raw["art_share"]),
            "documented_suppression": float(model_metrics_raw["suppressed_share"]),
            "second95": float(model_metrics_raw["second95"]),
            "suppressed_among_art": float(model_metrics_raw["suppressed_among_art"]),
        }
        reference_metrics = _point_reference_from_harp(holdout_point)
        reference_subset = {
            "diagnosed_stock": float(reference_metrics["diagnosed_stock"]),
            "art_stock": float(reference_metrics["art_stock"]),
            "documented_suppression": float(reference_metrics["documented_suppression"]),
            "second95": float(reference_metrics["second95"]),
            "suppressed_among_art": float(reference_metrics["suppressed_among_art"]),
        }
        comparator_metrics = {
            "diagnosed_stock": float(incumbent["diagnosed_plhiv"]) / max(float(incumbent["estimated_plhiv"]), 1.0),
            "art_stock": float(incumbent["alive_on_art"]) / max(float(incumbent["estimated_plhiv"]), 1.0),
            "documented_suppression": float(incumbent["virally_suppressed"]) / max(float(incumbent["estimated_plhiv"]), 1.0),
            "second95": float(incumbent["alive_on_art"]) / max(float(incumbent["diagnosed_plhiv"]), 1.0),
            "suppressed_among_art": float(incumbent["virally_suppressed"]) / max(float(incumbent["alive_on_art"]), 1.0),
        }
        model_errors = {
            f"{key}_abs_error": round(abs(model_metrics[key] - reference_subset[key]), 6)
            for key in reference_subset
        }
        incumbent_errors = {
            f"{key}_abs_error": round(abs(comparator_metrics[key] - reference_subset[key]), 6)
            for key in reference_subset
        }
        comparisons.append(
            {
                "holdout_year": holdout_year,
                "holdout_month": selected_month,
                "forecast_origin_month": forecast_origin_month,
                "incumbent_source_id": str(incumbent.get("source_id") or ""),
                "incumbent_source_label": str(incumbent.get("source_label") or ""),
                "incumbent_availability_month": str(incumbent.get("availability_month") or ""),
                "model_metrics": {key: round(float(value), 6) for key, value in model_metrics.items()},
                "incumbent_metrics": {key: round(float(value), 6) for key, value in comparator_metrics.items()},
                "holdout_reference": {key: round(float(value), 6) for key, value in reference_subset.items()},
                "model_errors": model_errors,
                "incumbent_errors": incumbent_errors,
            }
        )
    model_mean_absolute_error = round(
        float(np.mean([np.mean(list(row.get("model_errors", {}).values())) for row in comparisons if row.get("model_errors")]))
        if comparisons
        else 0.0,
        6,
    )
    incumbent_mean_absolute_error = round(
        float(np.mean([np.mean(list(row.get("incumbent_errors", {}).values())) for row in comparisons if row.get("incumbent_errors")]))
        if comparisons
        else 0.0,
        6,
    )
    return {
        "available": bool(comparisons),
        "reason": None if comparisons else "no_frozen_vintage_holdout_comparator_available",
        "comparator_family": "frozen_vintage_spectrum_aim_archive_reports",
        "profile_id": profile_id,
        "inference_family": inference_family,
        "forecast_origin_month": forecast_origin_month,
        "comparison_count": len(comparisons),
        "missing_holdout_years": missing_years,
        "model_mean_absolute_error": model_mean_absolute_error,
        "incumbent_mean_absolute_error": incumbent_mean_absolute_error,
        "model_beats_incumbent": bool(comparisons) and model_mean_absolute_error < incumbent_mean_absolute_error,
        "comparisons": comparisons,
    }
...
def _spectrum_style_comparison(
    *,
    ...
    train_window_rows = [row for row in comparisons if str(row.get("comparison_kind") or "") == "train_window"]
    holdout_rows = [row for row in comparisons if str(row.get("comparison_kind") or "") == "holdout"]
    train_window_mae = round(
        float(np.mean([np.mean(list(item.get("errors", {}).values())) for item in train_window_rows if item.get("errors")]))
        if train_window_rows
        else 0.0,
        6,
    )
    holdout_mae = round(
        float(np.mean([np.mean(list(item.get("errors", {}).values())) for item in holdout_rows if item.get("errors")]))
        if holdout_rows
        else 0.0,
        6,
    )
    frozen_vintage_holdout = _frozen_vintage_holdout_benchmark(
        run_id=run_id,
        profile_id=profile_id,
        inference_family=inference_family,
        province_axis=province_axis,
    ...
    return {
        ...
        "train_window_comparison_count": len(train_window_rows),
        "holdout_comparison_count": len(holdout_rows),
        "train_window_mean_absolute_error": train_window_mae,
        "holdout_mean_absolute_error": holdout_mae,
        "train_years": sorted(train_years),
        "holdout_years": sorted(holdout_years),
        "comparisons": comparisons,
        "frozen_vintage_holdout": frozen_vintage_holdout,
    }
...
def run_phase3_rescue_core(
    *,
    ...
    axis_catalogs = dict(axis_catalogs_override) if axis_catalogs_override is not None else read_json(ctx.run_dir / "phase1" / "axis_catalogs.json", default={})
    ...
    phase2_payload = load_phase2_compatibility_payload(ctx.run_dir)
    candidate_profiles = list(phase2_payload.get("candidate_profiles") or [])
    curated_candidate_blocks = list(phase2_payload.get("curated_candidate_blocks") or [])
    eligibility_surfaces = dict(phase2_payload.get("eligibility_surfaces") or {})
    direct_tensor = np.asarray(phase2_payload.get("direct_feature_tensor_array"), dtype=np.float32)
    hidden_driver_tensor = np.asarray(phase2_payload.get("hidden_driver_feature_tensor_array"), dtype=np.float32)
    multiscale_support_tensor = np.asarray(phase2_payload.get("multiscale_support_feature_tensor_array"), dtype=np.float32)
    ...
    dense_month_axis = _contiguous_month_axis(month_axis)
    standardized_tensor = _densify_standardized_tensor(np.asarray(standardized_tensor, dtype=np.float32), month_axis, dense_month_axis)
    month_axis = dense_month_axis
    axis_catalogs["month"] = month_axis
    ...
    archive_harp_points = _archive_harp_program_points(ctx.run_dir)
    archive_diagnosis_flow_points = _archive_diagnosis_flow_points(ctx.run_dir)
    effective_reference_overrides = dict(reference_overrides)
    if archive_harp_points and "harp_points" not in effective_reference_overrides:
        effective_reference_overrides["harp_points"] = archive_harp_points
    if archive_diagnosis_flow_points and "diagnosis_flow_points" not in effective_reference_overrides:
        effective_reference_overrides["diagnosis_flow_points"] = archive_diagnosis_flow_points
    with _temporary_reference_points(
        official_reference_points=effective_reference_overrides.get("official_points"),
        harp_program_points=effective_reference_overrides.get("harp_points"),
        diagnosis_flow_points=effective_reference_overrides.get("diagnosis_flow_points"),
    ):
        ...
        mixed_frequency_bundle = build_mixed_frequency_observation_bundle(
            month_axis=month_axis,
            province_axis=province_axis,
            support_bundle=observation_support_bundle,
            official_points=[dict(item) for item in OFFICIAL_REFERENCE_POINTS],
            harp_points=[dict(item) for item in HARP_PROGRAM_POINTS],
        ...
        if requested_inference_family in {"jax_svi", "jax_nuts"}:
            fit_result = _fit_rescue_core_jax_svi(
                ...
                mixed_frequency_bundle=mixed_frequency_bundle,
            )
    ...
    diagnosis_flow_summary = fit_result["parameters_summary"].get("diagnosis_flow_summary", {"point_count": 0, "rows": []})
    ...
    mixed_frequency_summary = {
        "rows": list(dict(mixed_frequency_bundle or {}).get("summary_rows", []) or []),
        "aggregation_modes": dict(dict(mixed_frequency_bundle or {}).get("aggregation_modes", {}) or {}),
    }
    ...
    fit_artifact = {
        ...
        "incumbent_comparator_summary": {
            ...
            "holdout_available": bool(dict(spectrum_comparison.get("frozen_vintage_holdout") or {}).get("available")),
            "holdout_comparison_count": int(dict(spectrum_comparison.get("frozen_vintage_holdout") or {}).get("comparison_count", 0)),
            "holdout_model_mean_absolute_error": float(dict(spectrum_comparison.get("frozen_vintage_holdout") or {}).get("model_mean_absolute_error", 0.0)),
            "holdout_incumbent_mean_absolute_error": float(dict(spectrum_comparison.get("frozen_vintage_holdout") or {}).get("incumbent_mean_absolute_error", 0.0)),
            "holdout_model_beats_incumbent": bool(dict(spectrum_comparison.get("frozen_vintage_holdout") or {}).get("model_beats_incumbent")),
        },
        ...
        "diagnosis_flow_summary": diagnosis_flow_summary,
        "phase4_ready": False,
        ...
        "mixed_frequency_observation_summary": mixed_frequency_summary,
    ...
    validation_artifact["diagnosis_flow_summary"] = diagnosis_flow_summary
    ...
    rescue_core_spec = {
        ...
        "modifier_representation": fit_result["parameters_summary"].get("determinant_covariates", {}).get("representation_mode"),
        "phase2_hook": {"mode": "benchmark_compatibility_only", "determinants_active": len(fit_result["parameters_summary"].get("determinant_covariates", {}).get("selected_determinant_modifiers", []))},
        "phase4_ready": False,
    }
    ...
    transition_probs_artifact = save_tensor_artifact(
        array=np.asarray(rollout_transition_probs, dtype=np.float32),
        axis_names=["province", "month", "transition"],
        artifact_dir=phase3_dir,
        stem="transition_probabilities",
        backend="torch" if torch is not None else "numpy",
        device=fit_result["device"],
        notes=["phase3_rescue_core_rollout_transition_probabilities"],
        save_pt=False,
    )
    ...
    write_json(phase3_dir / "diagnosis_flow_summary.json", diagnosis_flow_summary)
    write_json(phase3_dir / "observation_support_summary.json", observation_support_summary)
    write_json(phase3_dir / "mixed_frequency_observation_summary.json", mixed_frequency_summary)
    write_json(phase3_dir / "temporal_hazard_components.json", fit_result["parameters_summary"].get("temporal_hazard_components", {}))
    write_json(phase3_dir / "shock_regime_summary.json", fit_result["parameters_summary"].get("shock_regime_summary", {}))
    ...
    write_json(
        phase3_dir / "model_artifact.json",
        {
            ...
            "phase2_eligibility_surfaces": eligibility_surfaces,
            "direct_feature_shape": list(direct_tensor.shape),
            "hidden_driver_feature_shape": list(hidden_driver_tensor.shape),
            "multiscale_support_feature_shape": list(multiscale_support_tensor.shape),
        },
    )
    write_json(
    ...
    manifest = Phase0ManifestArtifact(
        ...
        artifact_paths={
            ...
            "transition_probabilities": transition_probs_artifact["value_path"],
            ...
            "mixed_frequency_observation_summary": str(phase3_dir / "mixed_frequency_observation_summary.json"),
            "temporal_hazard_components": str(phase3_dir / "temporal_hazard_components.json"),
            "shock_regime_summary": str(phase3_dir / "shock_regime_summary.json"),
    ...
    truth_paths = write_ground_truth_package(
        ...
        checks=[
            ...
            {"name": "mixed_frequency_observation_summary_present", "passed": bool(mixed_frequency_summary.get("rows", []))},
            {"name": "temporal_hazard_components_present", "passed": bool(fit_result["parameters_summary"].get("temporal_hazard_components", {}))},
            {"name": "shock_regime_summary_present", "passed": bool(fit_result["parameters_summary"].get("shock_regime_summary", {}))},
    ...
    ctx.record_stage_outputs(
        ...
        [
            ...
            phase3_dir / "mixed_frequency_observation_summary.json",
            phase3_dir / "temporal_hazard_components.json",
            phase3_dir / "shock_regime_summary.json",

...
def run_phase15_build(*, run_id: str, plugin_id: str, profile: str = PHASE15_PROFILE_ID) -> dict[str, Any]:
    ...
    gold_paths = write_gold_standard_package(
        ...
        checks=[
            ...
            {"name": "stability_report_present", "passed": bool(stability_rows)},
            {"name": "permutation_null_gap_reported", "passed": all("permutation_null_gap" in row for row in stability_rows)},
            {"name": "source_dropout_robustness_reported", "passed": all("source_dropout_robustness" in row for row in stability_rows)},
            {"name": "subnational_contrast_reported", "passed": all("region_contrast_score" in row for row in stability_rows)},
        ],
        stage_manifest_path=str(phase15_dir / "phase15_manifest.json"),
        summary={
            "factor_count": len(factor_catalog),
            "promotion_pool_count": len(promotion_pool),
    ...
    boundary_paths = write_boundary_shape_package(
        ...
        boundaries=[
            {
                "name": "mesoscopic_factor_tensor",
                "kind": "tensor",
                "path": str(phase15_dir / "mesoscopic_factor_tensor.npz"),
                "expected_shape": list(factor_tensor.shape),
                "expected_axis_names": ["province", "month", "factor"],
                "min_rank": 3,
                "finite_required": True,
            },
            {
                "name": "factor_loading_matrix",
                "kind": "tensor",
                "path": str(phase15_dir / "factor_loading_matrix.npz"),
                "expected_shape": list(loading_matrix.shape),
                "expected_axis_names": ["factor", "canonical_name"],
                "min_rank": 2,
                "finite_required": True,
            },
            {
                "name": "network_feature_tensor",
                "kind": "tensor",
                "path": str(phase15_dir / "network_feature_tensor.npz"),
                "expected_shape": list(network_tensor.shape),
                "expected_axis_names": ["province", "month", "network_feature"],
                "min_rank": 3,
                "finite_required": True,
            },
            {
                "name": "network_operator_tensor",
                "kind": "tensor",
                "path": str(phase15_dir / "network_operator_tensor.npz"),
                "expected_shape": list(operator_tensor.shape),
                "expected_axis_names": ["network_operator", "province", "province_neighbor"],
                "min_rank": 3,
                "finite_required": True,
            },
            {
                ...
                "min_rows": 1,
                "expected_row_count": len(factor_rows),
            },
            {
                "name": "factor_stability_report",
                "kind": "json_rows",
                "path": str(phase15_dir / "factor_stability_report.json"),
                "min_rows": 1,
                "expected_row_count": len(stability_rows),
            },
        ],
        summary={
            "factor_count": len(factor_catalog),
    ...
    ctx.record_stage_outputs(
        ...
        [
            ...
            phase15_dir / "gold_standard_checks.json",
            phase15_dir / "gold_standard_summary.json",
            phase15_dir / "boundary_shape_manifest.json",
            phase15_dir / "boundary_shape_checks.json",
            phase15_dir / "boundary_shape_summary.json",
